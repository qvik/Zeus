...
export const TabMetroExitsScreen = ({
  navigation,
}: RootTabScreenProps<"TabMetroExits">) => {
  const [selectedDestination, setSelectedDestination] = useState<string>("");
  const [avoid] = useState<string>(
    avoids[0].name + "|" + avoids[1].name + "|" + avoids[2].name
  );
  ...
  useEffect(() => {
    ...
    console.log(
      `exitsForSelectedStation is: ${JSON.stringify(exitsForSelectedStation)}`
    );
  }, [selectedStation]);
